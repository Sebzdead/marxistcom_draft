...
function parseMarkdown(mdText) {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    // Check YouTube Embed first to prevent it from being parsed as a generic image block
...
// ── AI Transcription Player Component ────────────────────────────────────────
function AIPlayer({ text }) {
  const [isPlaying, setIsPlaying] = useState(false);
  const [currentTime, setCurrentTime] = useState(0);

  const wordsCount = useMemo(() => (text ? text.split(/\s+/).length : 0), [text]);
  // average reading speed: 220 words per minute
  const duration = useMemo(() => Math.max(60, Math.ceil(wordsCount / 220) * 60), [wordsCount]);
  ...
  return (
    <div className="ai-player-container">
      <button
        className="ai-player-play-btn"
...
function App() {
  ...
  const formattedDate = useMemo(() => {
    if (!metadata.published) return "";
    try {
      const date = new Date(metadata.published);
      if (isNaN(date.getTime())) return metadata.published;
  ...
  return (
    <div className="site">
      ...
      <main className="site-main article-page-main">
        {loading ? (
          <div style={{ textAlign: "center", padding: "100px 0", fontFamily: "var(--rci-font-serif-display)", textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--rci-ash)" }}>
            Loading article...
          </div>
        ) : error ? (
          <div style={{ padding: "40px", border: "2px dashed var(--rci-red)", background: "var(--rci-paper-deep)", margin: "40px 0", textAlign: "center" }}>
            <Eyebrow style={{ fontSize: 14 }}>Error Loading Article</Eyebrow>
            <p style={{ fontFamily: "var(--rci-font-serif-display)", fontSize: 16, marginTop: 12 }}>{error}</p>
          ...
          <article className="article-body-container">
            ...
            {metadata.cover && (
              <div className="article-cover-wrap">
                <div className="article-cover-banner">
                  <img
                    src={metadata.cover}
                    alt={metadata.title}
            ...
            {/* Metadata Row below cover banner */}
            ...
            {/* Three-column layout grid: text + visualizer */}
            <div className="article-grid-columns">
              <div className="article-content">
                {/* AI transcription player element */}
                <AIPlayer text={articleContent} />
